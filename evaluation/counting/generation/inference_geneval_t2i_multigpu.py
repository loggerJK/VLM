...
def load_model(args, config, device):
    ...
    if args.checkpoint_path:
        ...
        unwrapped = os.path.join(ckpt, "unwrapped_model")
        if os.path.isdir(unwrapped):
            if os.path.exists(os.path.join(unwrapped, "adapter_config.json")):
                from peft import PeftModel
                model = MMadaModelLM.from_pretrained(base_path, trust_remote_code=True, torch_dtype=torch.bfloat16)
                model = PeftModel.from_pretrained(model, unwrapped)
            else:
                model = MMadaModelLM.from_pretrained(unwrapped, trust_remote_code=True, torch_dtype=torch.bfloat16)
        else:
            model = MMadaModelLM.from_pretrained(ckpt, trust_remote_code=True, torch_dtype=torch.bfloat16)
    else:
        model = MMadaModelLM.from_pretrained(base_path, trust_remote_code=True, torch_dtype=torch.bfloat16)
    ...
    for prompt_idx, metadata in my_prompt_items:
        ...
        for sample_idx in range(args.num_samples):
            ...
            if os.path.exists(sample_path):
                print(f"[rank {rank}] Skipping existing {sample_path}")
                continue
            ...
            print(f"[rank {rank}] Saved {sample_path}")
