#!/usr/bin/env python3
"""
PixMo Counting Evaluation Script — MMaDA (Single-GPU)
...
def extract_gt_number(text: str) -> int:
    """Extract ground-truth count (may be plain int or **N** format)."""
    match = re.search(r'\*\*(\d+)\*\*', text)
    if match:
        return int(match.group(1))
    match = re.search(r'\d+', text)
...
def main():
    ...
    parser.add_argument("--use_wandb", action="store_true",
                        help="If set, log eval metrics + confusion matrix to wandb")
    ...
    # Load dataset
    # ------------------------------------------------------------------
    print("Loading dataset Jiwon-Kang/pixmo-count-filtered-imgContained …")
    ...
    for i, item in tqdm(enumerate(dataset), total=501, desc="Evaluating"):
        set_all_seeds(args.seed)
        ...
        if gt_num < 0:
            try:
                ...
                pass
        if gt_num < 0:
            print(f"Warning: could not parse GT for item {i}: {gt_answer!r}")
            continue
    ...
    summary = {"accuracy": accuracy, "mad": mad, "total": len(results)}
    ...
    cm_full = confusion_matrix(true_labels, pred_labels, labels=valid_labels)
    fig2, ax2 = plt.subplots(figsize=(max(10, len(valid_labels)), max(8, len(valid_labels))))
    sns.heatmap(cm_full, annot=True, fmt='d', xticklabels=valid_labels, yticklabels=valid_labels,
                cmap='viridis', ax=ax2)
