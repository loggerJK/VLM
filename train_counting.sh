...
export WANDB_NAME="dset[pixmo_4-20]_ngpu2_bs1_accum64_lr4e-5_ep3_full"
NUM_GPUS=2
...
OUTPUT_DIR="./checkpoints/janus-counting"

# [설정] 학습 하이퍼파라미터
TUNING_MODE="full"      # 'lora' 또는 'full'
BATCH_SIZE=1         # Device당 배치 사이즈
EPOCHS=3
LR=4e-5
GRAD_ACCUM_STEPS=64      # Gradient Accumulation Steps
USE_GRAD_CHECKPOINT=0  # 1=True, 0=False (메모리 절약)
NUM_WORKERS=32          # Dataloader Workers
SAVE_STEPS=300           # 매 n 스텝마다 체크포인트 저장
LOG_FREQ=100             # 매 n 스텝마다 로그 기록
