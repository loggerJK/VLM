...
class ItemProcessorUnderstandingGeneration(ItemProcessorBase):
    ...
    def process_item(self, data_item: dict, training_mode=False, task='counting') -> Tuple[Any, str]:
        ...
        # Understanding Data
            ...
            if task == 'spatial_rel':
                question = data_item.get('question', data_item.get('text', ''))
                answer = str(data_item.get('fullAnswer', data_item.get('label', '')))
...
class Solver(FinetuneSolverBase):
    ...
    def get_args_parser(cls):
        ...
        # Task Argument
        parser.add_argument("--task", type=str, default="counting", choices=["counting", "pointing", "spatial_relation"], help="Task type for understanding")
    ...
    def _dataset_func(self):
        ...
        train_ds = load_dataset("heez/gqa_spatialrel_processed", split="train", streaming=False)  
        ...
        self.val_ds = load_dataset("heez/gqa_spatialrel_processed", split="validation", streaming=False)
        ...
        self.val_ds = self.val_ds.shuffle(seed=42).select(range(min(100, len(self.val_ds))))
        item_processor = self._item_processor_func(tokenizer=self.tokenizer, max_len=self.args.max_seq_len)
        
        if self.args.task == 'spatial_relation':
            train_gen_ds = None
        ...
        self.train_und_ds_val = train_und_ds.shuffle(seed=42).select(range(min(100, len(train_und_ds))))
    ...
    def validate(self, epoch, format="counting", split="val"):
        ...
        if self.global_rank == 0:
            print(f"\n[Epoch {epoch} | Step {self.global_step}] Running Validation ({format}) on {split} split...")
        ...
        pred_counts = []
        spatial_corrects = []
        spatial_table_rows = [] # Store rows for spatial_relation [idx, gt_full, gt_short, model_ans, correct]
        ...
        with torch.no_grad():
            ...
            for qidx, item in local_dataset_list:
                ...
                if format == 'spatial_relation':
                    question = item.get('question', '')
                    gt_answer_full = str(item.get('fullAnswer', ''))
                    gt_answer_short = str(item.get('answer', ''))
                    gt_count = 0 # Dummy for safety
                else:
                    if split == "train" and self.args.task == "counting":
                ...
                if format == 'pointing':
                    ...
                    question = (f"Locate all {label}. How many {label} are there in the image?. "
                                f"Response Example : {question_point_example}. {question_count_example}")

                if image is None or gt_count is None: continue
                ...
                instruction = f"<system>You are a multimodal model that can process both text and images. Answer the following question based on the provided images.</system><user>{question}</user>"
                ...
                GEN_LENGTH = 20 if (format == "counting" or format == "spatial_relation") else 512
                STEPS = 20 if (format == "counting" or format == "spatial_relation") else 128
                input_ids = torch.tensor(input_token + [BOA] + GEN_LENGTH * [MASK], device=f"cuda:{self.global_rank}").unsqueeze(0)
                
                out_tokens = generate_text_understanding(self.model, input_ids, steps=STEPS, gen_length=GEN_LENGTH, block_length=STEPS, temperature=0.0, cfg_scale=0.0, remasking='low_confidence', code_start=code_start)
                answer = self.tokenizer.batch_decode(out_tokens[:, code_start:], skip_special_tokens=True)[0]
                
                if format == 'spatial_relation':
                    is_correct = gt_answer_short.lower() in answer.lower()
                    spatial_corrects.append(1 if is_correct else 0)
                    spatial_table_rows.append([qidx, gt_answer_full, gt_answer_short, answer, "Correct" if is_correct else "False"])
                    res_str = f"[{qidx}] Q: {question} | GT: {gt_answer_short} | Ans: {answer} | Correct: {is_correct}"
                else:
                    pred_count = extract_number_fixed(answer)
                    is_correct = (int(pred_count) == int(gt_count))
                    gt_counts.append(int(gt_count))
                    pred_counts.append(int(pred_count))
                    res_str = f"[{qidx}] Q: {question} | GT: {gt_count} | Pred: {pred_count} | Correct: {is_correct}"
        ...
        all_spatial_rows = [None] * world_size
        all_spatial_corrects = [None] * world_size
        all_gt_counts = [None] * world_size
        all_pred_counts = [None] * world_size

        dist.barrier()
        dist.all_gather_object(all_details, details_buffer)
        dist.all_gather_object(all_spatial_rows, spatial_table_rows)
        dist.all_gather_object(all_spatial_corrects, spatial_corrects)
        dist.all_gather_object(all_gt_counts, gt_counts)
        dist.all_gather_object(all_pred_counts, pred_counts)

        if self.global_rank == 0:
            details_buffer = [d for sub in all_details for d in sub]
            spatial_rows = [r for sub in all_spatial_rows for r in sub]
            spatial_corrects = [c for sub in all_spatial_corrects for c in sub]
            gt_counts = [c for sub in all_gt_counts for c in sub]
            pred_counts = [c for sub in all_pred_counts for c in sub]

            log_data = {f"{split}/epoch": epoch, "global_step": self.global_step}

            if format == 'spatial_relation':
                accuracy = sum(spatial_corrects) / len(spatial_corrects) if spatial_corrects else 0
                log_data[f"{split}/accuracy_spatial_relation"] = accuracy
                if self.args.use_wandb:
                    table = wandb.Table(columns=["QuestionIdx", "GT(FullAnswer)", "GT(Answer)", "ModelAnswer", "Correct"])
                    spatial_rows.sort(key=lambda x: x[0]) # Sort by original index
                    for row in spatial_rows: table.add_data(*row)
                    log_data[f"{split}/predictions_spatial_relation"] = table
            else:
                total = len(gt_counts)
                correct = sum(1 for g, p in zip(gt_counts, pred_counts) if g == p and p >= 0)
                accuracy = correct / total if total > 0 else 0
                log_data[f"{split}/accuracy"] = accuracy
                # ... existing confusion matrix logic could go here ...
                if self.args.use_wandb:
                    table = wandb.Table(columns=["Step", "Accuracy", "Details"])
                    table.add_data(self.global_step, accuracy, "\n".join(details_buffer))
                    log_data[f"{split}/predictions"] = table
    ...
    def run(self):
        ...
        if self.args.mode in ['und', 'both']:
            val_format = "spatial_relation" if self.args.task == "spatial_relation" else "counting"
            self.validate(self.start_epoch, format=val_format, split="train")
            self.validate(self.start_epoch, format=val_format, split="val")
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            if (data_iter_step + 1) % accum_iter == 0:
                ...
                if self.global_step % self.args.validation_interval == 0:
                    if self.args.mode in ['und', 'both']:
                        val_format = "spatial_relation" if self.args.task == "spatial_relation" else "counting"
                        self.validate(epoch, format=val_format, split="train")
                        self.validate(epoch, format=val_format, split="val")
                        if self.args.validation_as_pointing_format:
