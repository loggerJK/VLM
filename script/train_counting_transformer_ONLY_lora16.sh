...
export WANDB_NAME="train[transformer_ONLY]_lora[16]_dset[pixmo-count]_ngpu1_bs1_accum128_lr4e-5"
NUM_GPUS=1

# RESUME 설정
export WANDB_RUN_ID="5hu2bzq0"  # 이전 실행의 WandB Run ID를 입력하세요.
export WANDB_RESUME="must"  # 'must', 'never', 'allow' 중 선택
export RESUME_GLOBAL_STEP=2800  # 체크포인트에서 재개할 스텝 번호
export RESUME_EPOCH=14  # 체크포인트에서 재개할 에폭 번호
export RESUME_CHECKPOINT_PATH="/mnt/data1/jiwon/deepseek-janus-pro-lora/checkpoints/train[transformer_ONLY]_lora[16]_dset[pixmo-count]_ngpu1_bs1_accum128_lr4e-5/checkpoint-2800"  # 재개할 체크포인트 경로

# [설정] 사전 학습된 Janus 모델 경로
# 실제 모델 가중치(config.json, pytorch_model.bin 등)가 있는 디렉토리로 변경해주세요.
MODEL_PATH="deepseek-ai/Janus-Pro-7B"
DATA_PATH="Jiwon-Kang/pixmo-count-filtered-imgContained"
OUTPUT_DIR="./checkpoints/${WANDB_NAME}"

# [설정] 학습 하이퍼파라미터
TUNING_MODE="transformer_lora"      # 'lora' 또는 'full'
LORA_R=16                   # LoRA Rank (TUNING_MODE가 'lora'일 때만 사용)
LORA_ALPHA=16                # LoRA Alpha (TUNING_MODE가 'lora'일 때만 사용)
...
echo "Resume Run ID   : $WANDB_RUN_ID"
echo "Resume Checkpoint Path : $RESUME_CHECKPOINT_PATH"
echo "Resume Global Step : $RESUME_GLOBAL_STEP"
