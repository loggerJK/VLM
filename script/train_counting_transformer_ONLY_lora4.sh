...
export WANDB_NAME="train[transformer_ONLY]_lora[4]_dset[pixmo-count]_ngpu1_bs1_accum128_lr4e-5"
NUM_GPUS=1

export TOKENIZERS_PARALLELISM=false

# RESUME 설정
export WANDB_RUN_ID="kv6c9ybb"  # 이전 실행의 WandB Run ID를 입력하세요.
export WANDB_RESUME="must"  # 'must', 'never', 'allow' 중 선택
export RESUME_GLOBAL_STEP=3290  # 체크포인트에서 재개할 스텝 번호
export RESUME_EPOCH=16  # 체크포인트에서 재개할 에폭 번호
export RESUME_CHECKPOINT_PATH="/mnt/data1/jiwon/deepseek-janus-pro-lora/checkpoints/train[transformer_ONLY]_lora[4]_dset[pixmo-count]_ngpu1_bs1_accum128_lr4e-5/checkpoint-3200"  # 재개할 체크포인트 경로
...
LORA_R=4                     # LoRA Rank (TUNING_MODE가 'lora'일 때만 사용)
LORA_ALPHA=4                # LoRA Alpha (TUNING_MODE가 'lora'일 때만 사용)
