...
# Usage:
#   bash train_ocr_synthetic.sh gen                              # Fresh start, all 8 GPUs
#   bash train_ocr_synthetic.sh gen output/epoch0_step-500       # Resume from explicit checkpoint
#   bash train_ocr_synthetic.sh both                             # Train both gen + und
#   CUDA_VISIBLE_DEVICES=4,5 bash train_ocr_synthetic.sh gen     # Use GPU 4,5 only
#
...
export OUTPUT_FOLDER=${OUTPUT_FOLDER:-/mnt/data1/jiwon/BLIP3o/output/ocr_synthetic_${MODE}}
export WANDB_PROJECT="blip3o-ocr"
export WANDB_NAME="train[lora128]_mode[${MODE}]_task[${TASK}]_dset[ocr_synthetic]_gpu${NUM_GPUS}"
