# -*- coding: utf-8 -*-
"""
OCR Generation Evaluation Script (Multi-GPU, 2-Stage)
...
def main():
    parser = argparse.ArgumentParser(description="OCR Generation Evaluation")
    ...
    for i, item in tqdm(enumerate(dataset), total=args.num_samples, desc=f"Rank {rank} Stage1"):
        ...
        prompt_text = (
            f"A clean white document with sharp, legible black printed text. "
            f"High-resolution typography, centered on the page, pure white background, "
            f"no decorations, professional document style, top-down view. "
