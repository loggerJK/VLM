# -*- coding: utf-8 -*-
"""
OCR Understanding Evaluation Script (Multi-GPU) — Janus
...
def main():
    parser = argparse.ArgumentParser(description="OCR Understanding Evaluation (Janus)")
    ...
    for i in tqdm(local_indices, desc=f"Rank {rank}"):
        ...
        pred_text = tokenizer.decode(outputs[0].cpu().tolist(), skip_special_tokens=True).strip()
