...
EVAL_BASE_DIR="/mnt/data1/dvlm/lumina/rel_position_v2/generation"
...
for ((i=${#parents[@]}-1; i>=0; i--)); do
    ...
    # Condition 1: Check if '00099' subdirectory exists
    if [ ! -d "$parent/00099" ]; then
        continue
    fi
    ...
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH="$BASE_DIR" conda run -n geneval python -u "$BASE_DIR/evaluation/evaluate_images_spatial_rel.py" \
        "$parent" \
