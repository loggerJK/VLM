# -*- coding: utf-8 -*-
"""
PixMo Counting Understanding Evaluation — Qwen2.5-VL with optional BLIP3o LoRA

Evaluates counting ability on synthetic counting images using Qwen2.5-VL as the
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Pixmo Counting (Qwen2.5-VL)")
    ...
    iterator = tqdm(enumerate(dataset), total=501, desc=f"Rank {rank}")
    ...
    for i, item in iterator:
        ...
        print("=" * 50)
        print(f"GT Answer: {gt_answer} (extracted number: {gt_num})")
        print(f"Pred Answer: {text_new} (extracted number: {pred_num})")
        print("=" * 50)
