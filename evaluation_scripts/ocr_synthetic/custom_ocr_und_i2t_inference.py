...
ds = load_dataset("Jiwon-Kang/OCR-Synthetic-Rendered-200K", split="validation", streaming=True)
...
final_wer = jiwer_wer(references, predictions)
acc = [ref == pred for ref, pred in zip(references, predictions)]
acc = sum(acc) / len(acc)
print(f"Final Accuracy: {acc:.4f}")
print(f"Final WER: {final_wer}")
# Save to file
with open("i2t_understanding_results.txt", "w") as f:
