# -*- coding: utf-8 -*-
"""
OCR Understanding Evaluation Script (Multi-GPU) — Janus (Synthetic Images)
...
def main():
    parser = argparse.ArgumentParser(description="OCR Understanding Evaluation — Synthetic (Janus)")
    ...
    if args.lora_ckpt_path is not None:
        if rank == 0:
            print(f"[INFO] Loading LoRA weights from {args.lora_ckpt_path}...")
    ...
    for i in tqdm(local_indices, desc=f"Rank {rank}"):
        ...
        pred_text = tokenizer.decode(outputs[0].cpu().tolist(), skip_special_tokens=True).strip().replace("\n", " ")
