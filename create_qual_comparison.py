...
# MASTER Metadata used by cvlab22 agent
METADATA_FILE = "/mnt/data1/jiwon/geneval/prompts/evaluation_metadata_count.jsonl"

MODELS = [
    {
        "path": "janus_pro_7b_baseline",
        "label": "Baseline"
    },
    {
        "path": "train_full_ckpt600",
        "label": "Full (Fine-tune)\nStep 600"
    },
    {
        "path": "train_transformer_ckpt600",
        "label": "Trans (Fine-tune)\nStep 600"
    },
    {
        "path": "train_transformer_only_count_ckpt12000",
        "label": "Trans Only (Count)\nStep 12000"
    },
    {
        "path": "train_transformer_only_point_count_ckpt15000",
        "label": "Trans Only (Concat)\nStep 15000"
    },
    {
        "path": "train_transformer_only_points_ckpt15000",
        "label": "Trans Only (Point 0-20)\nStep 15000"
    }
]
...
with open(METADATA_FILE, 'r') as f:
    for i, line in enumerate(f):
        ...
        # Key 'prompt' verified in evaluation_metadata_count.jsonl
        prompts[i] = data['prompt']

print(f"Loaded {len(prompts)} prompts.")

# --- Process ---
for prompt_idx in tqdm(range(len(prompts)), desc="Generating Grids"):
    prompt_id_str = f"{prompt_idx:05d}"
    ...
    safe_text = "".join([c if c.isalnum() else "_" for c in prompt_text])[:50]
    filename = f"{prompt_id_str}_{safe_text}.png"
