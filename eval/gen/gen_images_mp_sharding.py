...
if __name__ == "__main__":
    ...
    parser.add_argument('--lora-ckpt-path', type=str, default=None, help='LoRA checkpoint path (optional)')
    ...
    print(f"[Rank {rank}] Output images are saved in {output_dir}")
    ...
    print(f"[Rank {rank}] Loading model weights to {device}...")
    ...
    if args.lora_ckpt_path is not None:
        ...
        model = model.to(torch.bfloat16).to(device).eval()
        print(f"[Rank {rank}] Loaded LoRA weights from {args.lora_ckpt_path}")
    ...
    for idx in tqdm(local_indices, desc=f"Rank {rank} Processing", unit="prompt"):
        ...
        print(f"[Rank {rank}] Processing prompt {idx + 1}/{len(metadatas)}: '{prompt}'")
        ...
        for i in range(args.num_images):
            if not os.path.exists(os.path.join(sample_path, f"{i:05}.png")):
                ...
                break
        if all_generated:
            print(f"[Rank {rank}] Skipping generation for prompt: {prompt}")
            continue
    ...
    print(f"[Rank {rank}] All tasks completed.")
