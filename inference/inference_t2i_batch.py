...
def main():
    parser = argparse.ArgumentParser(description="Batch Text-to-image inference from file")
    parser.add_argument("--checkpoint", type=str, required=True, help="Fine-tuned checkpoint path")
    parser.add_argument("--prompt_file", type=str, required=True, help="Path to text file containing prompts (one per line)")
    ...
    parser.add_argument("--output_dir", type=str, default="results_text_to_image_batch", help="Output directory")
    ...
    for idx, prompt_text in enumerate(tqdm(prompts, desc="Generating Images")):
        ...
        # Generate filename (index + sanitized prompt snippet)
        safe_prompt = "".join(c for c in prompt_text if c.isalnum() or c in (' ', '_', '-')).replace(" ", "_")
