...
def process_dataset():
    ...
    TARGET_REPO_NAME = 'pixmo-points-filtered[0-20]-imgContained'
    HUB_TARGET_REPO_NAME = 'pixmo-points-filtered_0-20_imgContained'
    
    
    for split in ['train']:
        output_path = f"data/{TARGET_REPO_NAME}/{split}"
        if os.path.exists(output_path):
            ...
            target_repo = f"Jiwon-Kang/{HUB_TARGET_REPO_NAME}"
            print(f"Pushing dataset to Hugging Face Hub: {target_repo}...")
            dataset.push_to_hub(target_repo, split=split)
            continue
        
        dataset = load_dataset("allenai/pixmo-points", split=split)
        ...
        dataset_with_images = dataset.map(download_image, num_proc=64)

        # Filter out invalid rows, using input_columns to avoid decoding images
        dataset_filtered = dataset_with_images.filter(lambda x: x["valid"], num_proc=64)
