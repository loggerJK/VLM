...
class Trainer:
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        from functools import partial
        self._inner_training_loop_partial = partial(
            self._inner_training_loop,
            **kwargs,
        )
        inner_training_loop = find_executable_batch_size(
            self._inner_training_loop_partial, self._train_batch_size, args.auto_find_batch_size
        )
        if args.push_to_hub:
            try:
    ...
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None, **kwargs
    ):
        self.accelerator.free_memory()
        ...
        if kwargs.get("wandb_resume", False) in ['allow', 'must']:
            resume_global_step = kwargs.get("resume_global_step", 0)
            resume_epoch = kwargs.get("resume_epoch", 0)
            epochs_trained = resume_epoch
            self.state.global_step = resume_global_step
            self.state.epoch = resume_epoch
            steps_trained_in_current_epoch = 0
            logger.info("Resuming W&B run as per the `wandb_resume` argument passed to `train()`.")
            logger.info(f"  Resuming from epoch {epochs_trained}")
    ...
    def _save_checkpoint(self, model, trial):
        ...
        if self.args.save_strategy == SaveStrategy.EPOCH:
            checkpoint_folder = f"epoch-{math.ceil(self.state.epoch)}"
