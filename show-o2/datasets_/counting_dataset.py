...
from datasets_.utils import image_transform, resize_and_pad_image, to_tensor_and_normalize
import os
import json
...
class CountingDataset(data.Dataset):
    def __init__(
            self,
            root: str,
            text_tokenizer,
        ...
        self.num_image_tokens = num_image_tokens # 이미지 토큰 개수는 미리 고정
        ...
        # Load dataset logic similar to StreamingDatasetWrapper
        import os
        from datasets import load_dataset, load_from_disk
        
        # Check if root is a local path to a dataset or we should use the HF hub
        # Use annotation_path or root as the dataset path/name
        # For this specific request, we default to Jiwon-Kang/pixmo-count-filtered-imgContained
        # but allow override via annotation_path if provided and exists
        
        dataset_name_or_path = root
        if not dataset_name_or_path: 
             dataset_name_or_path = "Jiwon-Kang/pixmo-count-filtered-imgContained"
             

        print(f"Loading {split} dataset from {dataset_name_or_path}")
        try:
            if os.path.exists(dataset_name_or_path):
                self.dataset = load_from_disk(os.path.join(dataset_name_or_path, split))
                # if hasattr(self.dataset, 'keys') and split in self.dataset.keys():
                #     self.dataset = self.dataset[split]
            else:
                self.dataset = load_dataset(dataset_name_or_path, split=split)
        except Exception as e:
            print(f"Error loading dataset: {e}. Fallback to default HF dataset.")
            self.dataset = load_dataset("Jiwon-Kang/pixmo-count-filtered-imgContained", split=split)
        ...
        print(f"MMU Dataset loaded. {self.num_samples} items!")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if 'image' in item.keys() and item['image'] is not None:
                ...
                # Retry random if failed
                retry_count = 0
                while image is None and retry_count < 10:
                    random_idx = np.random.randint(0, self.num_samples)
                    item = self.dataset[random_idx] # Update item to random one
                    if 'image' in item.keys() and item['image'] is not None:
                        image = item['image']
                    else:
                        image = self.fetch_image(item.get('image_url'))
            ...
            if self.clip_processor is not None:
                image_tensor = self.clip_processor(images=[image], return_tensors="pt")['pixel_values'].squeeze(0)
                img_clip = None # Or process for clip if needed
            else:
                # resize and pad, not centercrop
                image_tensor = resize_and_pad_image(image, target_resolution=(self.image_size, self.image_size))
                img_clip = copy.deepcopy(image_tensor).resize((384, 384))
                img_clip = to_tensor_and_normalize(img_clip, mean=self.mean, std=self.std)
                image_tensor = to_tensor_and_normalize(image_tensor, mean=self.mean, std=self.std)

            # Text processing
            # Construct conversation for Show-o format
            # User: <image>\n{question}
            # Assistant: {answer}
            
            question = item.get('question', '')
            answer = item.get('answer', '')
            label = item.get('label', 'objects')
            
            # Add Response Example
            # question = f"{question} Response Example : There are **<number>** of {label} in the image."
            
            # Check if question already has <image>
            ...
            targets = [f"{answer}{self.text_tokenizer.eos_token}"]

            # Tokenize
            sources_tokenized = [self.text_tokenizer(
                source,
            ...
            targets_tokenized = [self.text_tokenizer(
                target,
            ...
            text_tokens, text_labels, modality_positions, text_mask, image_mask = \
                self.format_multi_sequence_und_qwen2_5(sources_tokenized, targets_tokenized)
    ...
    #                    conv["role"] == "assistant"]
    #         print(f"[DEBUG] sources: {sources}, targets: {targets}")
