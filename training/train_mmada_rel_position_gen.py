...
#
# T2I rel-position generation fine-tuning script for MMaDA.
# Based on train_mmada_counting_gen.py; same T2I-only structure but uses
# heez/relative-position-new with the `answer` column as the caption,
# mirroring Lumina-DiMOO `task=rel_position, mode=gen`
# (`train_unified.py:1093-1099` maps `answer` -> `descriptions`).
#
# Key design notes (mirror of counting_gen, see plans/mmada-spatial-rel-position-gen-staged-manatee.md):
#   - Uses new RelPositionGenDataset (no count filter, all rows are gen).
...
#   - Validation: rel_position validation split; wandb image table logging
#     (no auto position metric — visual logging only, Lumina parity).
...
def main():
    ...
    # ------------------------------------------------------------------
    # Dataloaders (T2I-only via RelPositionGenDataset)
    # ------------------------------------------------------------------
    ...
    assert config.training.batch_size_t2i > 0, (
        "T2I-only rel_position_gen training requires batch_size_t2i > 0"
    )
    assert config.training.batch_size_lm == 0, "batch_size_lm must be 0 in rel_position_gen"
    assert config.training.batch_size_mmu == 0, "batch_size_mmu must be 0 in rel_position_gen"

    dataset_rel_position_gen = RelPositionGenDataset(
    ...
    train_dataloader_t2i = DataLoader(
        dataset_rel_position_gen,
        batch_size=config.training.batch_size_t2i,
        sampler=None,
        collate_fn=dataset_rel_position_gen.collate_fn,
        num_workers=dataset_config.num_workers,
    )
    num_gen_samples = dataset_rel_position_gen.num_samples
    num_update_steps_per_epoch = max(1, num_gen_samples // total_batch_size)
    num_train_epochs = math.ceil(config.training.max_train_steps / num_update_steps_per_epoch)
    logger.info(
        f"rel-position-gen size: {num_gen_samples}, steps/epoch: {num_update_steps_per_epoch}, "
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, batch_idx, dataloader_idx in combined_dataloader:
            ...
            # T2I (only flow)
            assert "t2i_flow" in batch, "t2i_flow missing — rel_position_gen requires it"
            ...
            if global_step == 0 and accelerator.is_main_process:
                ...
                print("=== T2I REL-POSITION-GEN SANITY (sample 0) ===")
...
# ---------------------------------------------------------------------------
# RelPosition-Gen validation (validation split, image generation -> wandb table)
# ---------------------------------------------------------------------------
...
def validate_rel_position_gen(
    ...
    """Generate images for validation captions and log a wandb image table.

    Mirrors Lumina-DiMOO `log_validation_images` (no auto position-accuracy metric).
    ...
    if is_main:
        logger.info("Running rel-position-gen validation...")
    model.eval()

    if not val_items:
        if is_main:
            logger.warning("validate_rel_position_gen: empty val_items, skipping.")
        model.train()
        return
    ...
    if is_main:
        print("=" * 70)
        print(
            f"RelPosition-Gen validation @ step {global_step}  "
    ...
    iterator = tqdm(
        ...
        desc=f"RelPosition-Gen val rank {rank}",
        position=rank,
        leave=True,
        dynamic_ncols=True,
    )
    ...
    for sample_idx, item in iterator:
        try:
            # Lumina parity: caption := answer column.
            ...
            gt_position = str(item.get('position', ''))
            ...
            with torch.autocast("cuda", dtype=weight_dtype, enabled=accelerator.mixed_precision != "no"):
                gen_token_ids = accelerator.unwrap_model(model).t2i_generate(
                    ...
                    # our msl=128 (rel_position) the default would slice into image tokens
                    # and corrupt CFG.
                    resolution=msl,
            ...
            print(
                f"[rank {rank}] sample={sample_idx:>3} gt_position={gt_position:>12} "
                f"caption={caption[:80]!r}{'...' if len(caption) > 80 else ''}"
            )
        except Exception as e:
            logger.warning(f"RelPosition-gen val error rank {rank} idx {sample_idx}: {e}")
            continue
    ...
    if not results:
        logger.warning("validate_rel_position_gen: no results gathered.")
        model.train()
        return

    pred_table = wandb.Table(columns=["sample_idx", "caption", "gt_position", "gt_image", "gen_image"])
    for r in results:
        gt_img_obj = (
        ...
        pred_table.add_data(r["sample_idx"], r["caption"], r["gt_position"], gt_img_obj, gen_img_obj)

    wandb.log({
        "rel_position_gen/val_images": pred_table,
    }, step=global_step)

    print("-" * 70)
    print(f"Logged {len(results)} rel-position-gen validation samples to wandb @ step {global_step}")
    print("=" * 70)

    logger.info(
        f"RelPosition-gen validation step {global_step}: logged {len(results)} samples"
    )
